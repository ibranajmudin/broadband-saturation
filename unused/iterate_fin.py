...
def iterate_fin(lambda_in, sigma_in):
    # PARAMS
    ...
    for i in range(0, its):
        ...
        # aside : use this conditional to only plot the final pulse
        if i == its-1:
            ...
            axs.plot(lambda_in*1e9, (intensity_spec_in[imin]*1e-9)/intensity_tot_in[imin], color="b")
            axs.plot(lambda_in*1e9, (intensity_spec_in[imax]*1e-9)/intensity_tot_in[imax], color="g")
        ...
        #     axs.plot(lambda_in*1e9, intensity_spec_in_avg*1e-9*1e-4, color="r")
    ...
    # axs.set_ylabel(r"$F(\lambda)$ [/nm]")
    axs.set_title("Normalised input spectral profiles", fontsize=10)
    axs.legend(["front", "back"])
